...
export const useChannelsAndDMs = (onSelectChannel, onSelectDM) => {
  ...
  useEffect(() => {
    const fetchChannelsAndDms = async () => {
      try {
        const channelsResponse = await axiosInstance.get('/api/channels');  // 채널 데이터 요청
        const dmsResponse = await axiosInstance.get('/api/dms');  // DM 데이터 요청
  ...
  const addChannel = async (channelName) => {
    try {
      await axiosInstance.post('/api/channels', { name: channelName });  // 서버에 채널 추가 요청
